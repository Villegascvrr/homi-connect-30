...
export const ImageUpload: React.FC<ImageUploadProps> = ({
  ...
  return <div className={className}>
      ...
      {multiple ? <div className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            {images.map((image, index) => <div key={index} className="relative group aspect-square rounded-md overflow-hidden bg-muted">
                <img src={image} alt={`Imagen ${index + 1}`} className="w-full h-full object-cover" />
                <button onClick={() => handleRemoveImage(index)} className="absolute inset-0 flex items-center justify-center bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity" type="button">
          ...
          {images.length > 0 ? <div className="relative group">
              ...
              <div className="absolute bottom-0 right-0 flex gap-2">
                ...
                <Button type="button" size="icon" variant="destructive" className="h-8 w-8 rounded-full" onClick={() => handleRemoveImage(0)}>
                  <X size={14} />
                </Button>
              </div>
            </div> : <div className="flex flex-col items-center gap-3">
              <Button type="button" onClick={handleClickUpload} disabled={isUploading} className="w-24 h-24 md:w-32 md:h-32 rounded-full flex items-center justify-center border-2 border-dashed border-primary/20 bg-muted hover:border-primary/50 transition-colors">
                <Camera size={32} className="text-muted-foreground" />
              </Button>
              <Button type="button" variant="outline" onClick={handleClickUpload} disabled={isUploading} className="mt-2">
      ...
      <Dialog open={isCropperOpen} onOpenChange={setIsCropperOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="flex flex-col items-center space-y-4">
            {selectedImage && <div className="relative w-64 h-64 mx-auto overflow-hidden border rounded-md">
                <img src={selectedImage} alt="Imagen a recortar" className="w-full h-full object-contain" style={{
