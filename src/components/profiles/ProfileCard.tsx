        ...
        <div className="p-5">
          <div className="flex justify-between items-start mb-2">
            <div>
              ...
              {username && <p className="text-sm text-muted-foreground flex items-center gap-1 mb-1">
          ...
          <div className="flex flex-wrap gap-1.5 my-3">
            {tags.slice(0, 4).map(tag => <span key={tag.id} className="px-2.5 py-0.5 text-xs rounded-full bg-homi-ultraLightPurple text-homi-purple">
                {tag.name}
              </span>)}
            {tags.length > 4 && <span className="px-2.5 py-0.5 text-xs rounded-full bg-homi-ultraLightPurple text-homi-purple">
                +{tags.length - 4}
              </span>}
          </div>
          
          <div className="flex gap-2 mt-4">
            <Button variant="outline" size="sm" className="rounded-full flex-1" onClick={() => onView && onView(id)}>
