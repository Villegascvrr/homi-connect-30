...
const Hero = () => {
  ...
  const renderContent = () => {
    // If user is not logged in, show registration options
    if (!user && !justRegistered) {
      return (
        <>
    ...
    // If user is logged in AND just registered, show welcome message
    if (user && justRegistered) {
      return <WelcomeMessage firstName={user?.user_metadata?.firstName || user?.user_metadata?.first_name || user?.user_metadata?.name || user?.user_metadata?.full_name} />;
    }
    
    // Otherwise, show regular logged-in user view
    return (
      <>
